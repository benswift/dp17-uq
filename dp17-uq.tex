...
results~\parencite{colella_participatory_2000,lui_supporting_2014}\todo{there
  are heaps more refs we could put in here if we needed to}.
...
events more common, there is a significant need for better ways to
...
inputs to the optimisation algorithm) which must be selected by the
...
choosing a parameter update $\delta p$ at each step (see
...
involves using a sparse grids approach combined with live modification
...
\todo[inline]{this should be a ``what is UQ'' section}
...
\todo[inline]{I'm not sure it's worth mentioning inverse problems in
  such detail. Given that we're not really that clear on how the
  forward problem fits in.}
...
The personnel involved in this project will be the CIs from the two ANU colleges, two post-doctoral fellows and several (four?) PhD students. % I have excluded Markus here
...
% Brendan: to some extent I have copied/reworked this section from the linkage project with Fujitsu, perhaps someone from computer science could add some good comp-sci journals to this list that would be relevant to the live-programming and interactive visualisation sides of the project. Possibly also mention something about making software available, open-source???
...
Software developed will be made available as open source??? Surrogate/reduced basis models to be made available online?

While communication in international media is essential for the project, we will also focus on Australian conferences and journals to further convince Australian researchers and organisations of the benefits and importance of live parameter space exploration for large scale scientific models.
