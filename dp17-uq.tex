...
\todo[inline]{\textbf{One idea:} we open with a brief vignette about
  ...
  know any folks who were involved?}
...
modelling?) this is often not the case. \todo[inline]{rather than the
  generic appeal to ``in the real world\ldots'', we could say ``we
  limit our analysis to situations where''\ldots}
\begin{enumerate}
...
\item performing the modelling step
  ...
  while still providing adequate fidelity
\item presenting the quantities $\mathbf{p}$,
  $u_{\mathbf{p}}(\mathbf{x})$ and $Q(u_{\mathbf{p}})$ to the
  scientist (and take interactive input) in a way which provides
  meaningful insight
\end{enumerate}
...
problems more tractible\todo{Brendan add refs}. Sparse grid methods
have also proved to be highly successful in the study and computation
of high dimensional problems\todo{Brendan add refs}.
