...
\begin{itemize}
\item in a manufacturing process, the
  relationship between the quality of the input materials and the
  defect rate in the finished products
\item in engineering and design, the relationship between an airfoil's
  shape (at both micro and macro scales) and its aerodynamic
  properties
...
\item in disaster response scenarios, the impact of fire/flood threats
  based on current sensor readings and reports from the field
...
parameterised partial differential equation (PPDE), such as in a storm
